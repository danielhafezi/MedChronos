...
/**
 * Enhance MedGemma caption to make it more readable using Gemini 2.5 Flash
 */
export async function enhanceMedGemmaCaption(
  rawCaption: string,
  sliceIndex: number,
  totalSlices: number
): Promise<string> {
  try {
    const prompt = `You are a medical imaging specialist. Take the following raw medical image caption generated by an AI model and rewrite it to be more readable and comprehensive while maintaining medical accuracy. 

Raw Caption: ${rawCaption}

Context: This is slice ${sliceIndex + 1} of ${totalSlices} total slices in the imaging study.
...
3. Notes the imaging characteristics (contrast, quality, orientation)
4. Maintains medical accuracy without hallucination
5. Is concise but comprehensive (2-4 sentences)

Do NOT add information that isn't present in the original caption. Focus on making the existing information more accessible and well-organized.
...
Enhanced Caption:`
    ...
    console.error('Error enhancing caption with Gemini Flash:', error)
    // Fall back to original caption if enhancement fails
    return rawCaption
  }
}

/**
 * Generate a comprehensive study summary using enhanced captions
 */
export async function generateEnhancedStudySummary(
  enhancedCaptions: string[],
  studyTitle: string,
  modality?: string
): Promise<string> {
  try {
...
- Number of slices: ${enhancedCaptions.length}

Enhanced Slice Descriptions:
${enhancedCaptions.map((caption, index) => `Slice ${index + 1}: ${caption}`).join('\n\n')}
...
Study Summary:`
    ...
    console.error('Error generating enhanced study summary:', error)
    throw new Error('Failed to generate enhanced study summary')
  }
}
